...
export type CommentaryLanguage = 'vi' | 'en';

export interface DirectorStyleProfile {
  id: string;
  name: string;
  badge: string;
  description: string;
  primaryModes: CameraMode[];
  secondaryModes: CameraMode[];
  primaryWeight: number; // Tỷ trọng ưu tiên nhóm góc quay chính (0.7 - 0.85)
  switchMin: number;
  switchMax: number;
}
...
export interface SystemConfig {
  ...
  fps: FPSOption;
  carsPerRace?: 0 | 6 | 8 | 10 | 12 | 15; // Số lượng xe đua cùng nhau (0 = Random mỗi luồng 8-16 xe, hoặc 6, 8, 10, 12, 15)
  ...
  commentaryLanguage?: CommentaryLanguage; // 'vi' | 'en'
  directorScriptMode?: 'per_instance_auto' | 'f1_broadcast' | 'action_cinema' | 'cockpit_driver' | 'sky_drone' | 'overtake_hunter' | 'spectator_grandstand';
}
