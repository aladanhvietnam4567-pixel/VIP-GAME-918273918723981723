...
class AudioEngine {
  ...
  generateRacingAudioPCM(
    ...
    const activeLang = language || (typeof localStorage !== 'undefined' && localStorage.getItem('commentary_language') === 'en' ? 'en' : 'vi');
    // Lấy kịch bản các đoạn bình luận chuẩn cho luồng này theo ngôn ngữ đã chọn
    const timeline = commentarySoundManager.getTimelineForInstance(instanceId, seed, durationSeconds, activeLang, participatingDrivers);
