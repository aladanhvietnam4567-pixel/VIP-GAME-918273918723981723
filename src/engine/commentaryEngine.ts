...
class CommentaryEngine {
  ...
  constructor() {
    try {
      ...
      const savedLang = localStorage.getItem('commentary_language');
  ...
  public toggleLanguage(): 'vi' | 'en' {
    const next = this.language === 'vi' ? 'en' : 'vi';
    this.setLanguage(next);
    return next;
  }

  private updateSpeakerName() {
    this.speakerName = this.language === 'en' ? 'F1 World Broadcast' : 'BLV Trẻ Trâu';
  }
  ...
  public triggerEvent(
    ...
    const clip = commentarySoundManager.getRandomClip(soundType, Math.floor(Math.random() * 100), this.language);
    
    let text = customText;
    if (!text) {
      const dyn = generateDynamicCommentary({
        category,
        driverName: options?.driverName || clip.driver || (this.language === 'en' ? 'Lewis Hamilton' : 'Cristiano Ronaldo'),
