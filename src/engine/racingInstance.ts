...
function customizeBiomeBySeed(baseBiome: TrackBiome, seed: number, instanceId: number = 1): TrackBiome {
  ...
  // 1. Loại kết cấu bề mặt đường đua (Road Surface Texture)
  const ROAD_TEXTURES = ['asphalt_dark', 'tarmac_grey', 'wet_reflection', 'red_f1', 'desert_clay', 'cyber_midnight', 'alpine_ice'] as const;
  biome.roadTextureType = ROAD_TEXTURES[(Math.abs(seed + instanceId * 3)) % ROAD_TEXTURES.length];
  ...
  // 2. Kiểu dáng cột đèn chiếu sáng (6 phong cách kiến trúc khác nhau cho mỗi luồng)
  const POLE_TYPES = ['curved_arch', 'double_cantilever', 'stadium_floodlight', 'minimalist_l', 'industrial_truss', 'cyber_disc'] as const;
  biome.lampPoleType = POLE_TYPES[(Math.abs(seed + instanceId * 5)) % POLE_TYPES.length];
  ...
  biome.poleColor = POLE_COLORS[(Math.abs(seed + instanceId * 2)) % POLE_COLORS.length];

  // 4. Màu ánh sáng đèn đường (Warm Halogen, Ice Blue, Bright White, Neon Cyan, Cyber Magenta, Blaze Orange)
  const LAMP_COLORS = [0xffffff, 0x38bdf8, 0xfbbf24, 0x22d3ee, 0xf43f5e, 0xa855f7, 0x4ade80, 0xf97316];
  biome.lampColor = LAMP_COLORS[(Math.abs(seed + instanceId * 4)) % LAMP_COLORS.length];

  // 5. Kiểu hoa văn vạch kẻ đường ở giữa (Single, Double, Triple, Pulse, Arrows, Dashed Yellow)
  const LINE_PATTERNS = ['single', 'double', 'triple', 'pulse', 'arrows', 'dashed_yellow'] as const;
  biome.centerLinePattern = LINE_PATTERNS[(Math.abs(seed + instanceId * 3)) % LINE_PATTERNS.length];

  // 6. Màu sắc vạch kẻ giữa đường
  const LINE_COLORS = [0xffffff, 0xfacc15, 0x22d3ee, 0x4ade80, 0xfb923c, 0xf43f5e];
  biome.centerLineColor = LINE_COLORS[(Math.abs(seed + instanceId)) % LINE_COLORS.length];
  ...
  biome.edgeLineColor = EDGE_COLORS[(Math.abs(seed + instanceId * 6)) % EDGE_COLORS.length];

  // 8. Cặp màu gờ giảm tốc (Curbs) độc bản theo từng chặng đua
  const KERB_COMBOS = [
    { c1: 0xffffff, c2: 0xdc2626 }, // Grand Prix Red/White
    { c1: 0xfacc15, c2: 0x0f172a }, // Hazard Yellow/Black
    { c1: 0xffffff, c2: 0x2563eb }, // Monaco Blue/White
    { c1: 0xffffff, c2: 0x16a34a }, // Monza Green/White
    { c1: 0x06b6d4, c2: 0xec4899 }, // Cyber Cyan/Magenta
    { c1: 0xf97316, c2: 0x1e293b }, // McLaren Orange/Black
    { c1: 0xa855f7, c2: 0xfacc15 }, // Royal Purple/Yellow
    { c1: 0xd97706, c2: 0x020617 }  // Gold/Stealth
  ];
  const chosenKerb = KERB_COMBOS[(Math.abs(seed + instanceId * 7)) % KERB_COMBOS.length];
  ...
  biome.bollardReflectorColor = REFLECTOR_COLORS[(Math.abs(seed + instanceId * 5)) % REFLECTOR_COLORS.length];
  biome.bollardBodyColor = [0x1e293b, 0xfacc15, 0xf8fafc, 0x0f172a][(Math.abs(seed + instanceId * 2)) % 4];
...
const DRIVER_NAMES = [
  // F1 Modern Champions & Superstars
  ...
  // F1 All-Time Legends & World Champions
...
export class RacingInstance {
  ...
  constructor(
    ...
    // Nếu carCount không truyền vào hoặc bằng 0 (Random Luồng), mỗi luồng 8 - 16 xe độc bản
    if (carCount && carCount > 0) {
      this.desiredCarCount = Math.max(4, Math.min(16, carCount));
    } else {
      this.desiredCarCount = 8 + (Math.abs(actualSeed * 13 + id * 7) % 9);
    }

    this.scene = new THREE.Scene();
    this.cameraDirector = new CameraDirector(68, 9 / 16, id);
  ...
  private initRace(customSeed?: number) {
    ...
    // Phân bổ 36 kiểu đường đua hoàn toàn khác nhau cho từng luồng (6 hoặc 8 luồng hoàn toàn không trùng nhau)
    const layoutIndex = ((this.id - 1) * 4 + Math.floor(seed / 7)) % ROAD_LAYOUT_PRESETS.length;
    ...
    // Phân bổ đạo diễn điện ảnh ngẫu nhiên riêng cho từng luồng (góc nhìn & nhịp chuyển cảnh khác nhau)
    ...
    const initialMode = INITIAL_CAMERA_MODES[(this.id * 3 + Math.abs(seed)) % INITIAL_CAMERA_MODES.length];
    ...
    this.seedData = {
      ...
      biome,
      weather: 'Sunny',
      roadLayout: biome.roadLayoutType || 'GRAND_PRIX_OVAL',
  ...
  private rebuildCars(seed: number, _biome: TrackBiome) {
    ...
    // Lấy chính xác ngẫu nhiên từ 6 đến 15 tay đua trong 60 siêu sao cầu thủ thế giới không trùng lặp
    const selectedDrivers = getParticipatingFootballDrivers(numCars, seed, this.id);
    ...
    for (let i = 0; i < numCars; i++) {
      ...
      // Tay đua chính xác từ 60 siêu sao bóng đá thế giới
      const driver = selectedDrivers[i] || selectedDrivers[i % selectedDrivers.length];
      const meshIdx = i % 5;
  ...
  recycleToNextRace(durationSeconds?: number, carsPerRace?: number) {
    ...
    if (carsPerRace !== undefined && carsPerRace > 0) {
      this.desiredCarCount = Math.max(4, Math.min(16, carsPerRace));
    } else {
      // Tự động random số lượng xe mới cho vòng đua tiếp theo (8 - 16 xe)
      this.desiredCarCount = 8 + (Math.abs(Date.now() * 13 + this.id * 7) % 9);
    }
  ...
  getRuntimeState(): InstanceRuntime {
    ...
    return {
      ...
      directorStyleName: this.cameraDirector?.getDirectorProfile?.()?.name || 'F1 World Feed',
      directorStyleBadge: this.cameraDirector?.getDirectorProfile?.()?.badge || '🎥 F1 World Feed'
    };
  }
}
