...
import { CameraMode, DirectorStyleProfile } from '../types';
import { Car3DObject } from './vehiclePhysics';
import { safeGetPointAt } from './curveUtils';

export const DIRECTOR_STYLE_PROFILES: DirectorStyleProfile[] = [
  {
    id: 'f1_world_feed',
    name: 'F1 World Feed Broadcast',
    badge: '🎥 F1 World Feed',
    description: 'Phong cách đài truyền hình F1 quốc tế: Trực thăng Helicam, Telephoto 85mm ven đường, Bám đoàn xe trên cao',
    primaryModes: [
  ...
  },
  {
    ...
    name: 'Hollywood Action Cinema',
    badge: '🎬 Hollywood Action',
    description: 'Phong cách bom tấn hành động: Sát mặt đường, Cản trước xé gió, Cận cảnh va chạm drift',
    primaryModes: [
      CameraMode.LOW_GROUND,
      CameraMode.OVERTAKE_ACTION,
  ...
  {
    id: 'cockpit_driver',
    name: 'Driver Cockpit & On-Board POV',
    badge: '🏎️ Cockpit POV',
    description: 'Góc nhìn thứ nhất trong buồng lái, nắp capo và đuôi gió siêu tốc',
    primaryModes: [
  ...
  {
    id: 'sky_drone_fpv',
    name: 'Aerial Sky Drone & Helicam',
    badge: '🚁 Sky Drone FPV',
    description: 'Racing Drone bay lượn trên cao và trực thăng truyền hình lượn quanh khúc cua',
    primaryModes: [
  ...
  {
    id: 'overtake_hunter',
    name: 'Overtake Hunter & Slipstream',
    badge: '⚔️ Overtake Hunter',
    description: 'Chuyên gia săn các pha vượt mặt, so kè tay đôi, núp gió tạt đầu nghẹt thở',
    primaryModes: [
      CameraMode.OVERTAKE_ACTION,
  ...
  {
    id: 'trackside_spectator',
    name: 'Trackside Spectator & Grandstand',
    badge: '🏟️ Trackside Spectator',
    description: 'Góc nhìn người xem từ khán đài, vách kỹ thuật Pit Wall và đỉnh góc cua Apex',
    primaryModes: [
      CameraMode.TRACKSIDE_TELEPHOTO,
  ...
  {
    id: 'orbit_showcase',
    name: '360° Cinematic Orbit Showcase',
    badge: '🔄 360° Orbit Showcase',
    description: 'Xoay 360 độ quanh siêu xe, tôn vinh khí động học và vệt khói lốp',
    primaryModes: [
  ...
  {
    id: 'extreme_speed',
    name: 'Extreme Low-Ground & Aerodynamics',
    badge: '⚡ Extreme Speed',
    description: 'Góc siêu thấp giữa tim đường lùi 10m ngắm xe lao 500-650 km/h',
    primaryModes: [
      CameraMode.LOW_GROUND,
      CameraMode.BUMPER_FIRST_PERSON,
  ...
  {
    id: 'grand_prix_live',
    name: 'Grandstand Live Show Panoramic',
    badge: '🎪 Grand Prix Live',
    description: 'Toàn cảnh không khí lễ hội tốc độ, đài cao và đón đầu đoàn xe',
    primaryModes: [
      CameraMode.PANORAMIC,
      CameraMode.MULTI_CAR_FRONT_FACING,
      CameraMode.CHOPPER_HELI_CHASE,
  ...
  {
    id: 'hybrid_master',
    name: 'Dynamic Grand Prix Hybrid Director',
    badge: '👑 Hybrid Master',
    description: 'Tổng đạo diễn thông minh: Kết hợp luân phiên F1 Broadcast và Hollywood Action',
    primaryModes: [
      CameraMode.MULTI_CAR_PACK_CHASE,
      CameraMode.LOW_GROUND,
      CameraMode.CHOPPER_HELI_CHASE,
...
export function getDirectorProfileForInstance(instanceId: number, customId?: string): DirectorStyleProfile {
  if (customId) {
    const found = DIRECTOR_STYLE_PROFILES.find(p => p.id === customId);
    if (found) return found;
  }
  const idx = ((Math.max(1, instanceId) - 1) % DIRECTOR_STYLE_PROFILES.length);
  return DIRECTOR_STYLE_PROFILES[idx];
}
...
export class CameraDirector {
  ...
  public directorProfile: DirectorStyleProfile;
  ...
  constructor(fov: number = 68, aspect: number = 16 / 9, instanceId: number = 1) {
    this.directorProfile = getDirectorProfileForInstance(instanceId);
    ...
    if (this.directorProfile.primaryModes.length > 0) {
      this.currentMode = this.directorProfile.primaryModes[0];
    }
  }

  setDirectorProfile(profile: DirectorStyleProfile) {
    this.directorProfile = profile;
    this.resetFirstFrame();
  }

  getDirectorProfile(): DirectorStyleProfile {
  ...
  /**
   * Chuyển đổi góc quay tự động theo Kịch Bản Đạo Diễn Riêng Biệt của Luồng (Director Profile)
   * Từng luồng đua sở hữu cá tính điện ảnh và nhịp điệu chuyển góc hoàn toàn độc nhất!
   */
  private cycleNextCinematicMode() {
    const prof = this.directorProfile || DIRECTOR_STYLE_PROFILES[0];
    let chosenPool: CameraMode[];
    const minTime = prof.switchMin || 4.5;
    const maxTime = prof.switchMax || 6.5;
    const nextDuration = minTime + Math.random() * (maxTime - minTime);

    // Ưu tiên nhóm góc quay chủ đạo của kịch bản đạo diễn
    if (Math.random() < prof.primaryWeight && prof.primaryModes.length > 0) {
      chosenPool = prof.primaryModes;
    } else if (prof.secondaryModes && prof.secondaryModes.length > 0) {
      chosenPool = prof.secondaryModes;
    } else {
      chosenPool = prof.primaryModes.length > 0 ? prof.primaryModes : CameraDirector.BROADCAST_MULTI_CAR_MODES;
    }
    ...
    if (available.length > 0) {
      ...
      this.currentMode = chosenPool[Math.floor(Math.random() * chosenPool.length)];
    }
