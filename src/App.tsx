...
export default function App() {
  ...
  const [config, setConfig] = useState<SystemConfig>({
    ...
    carsPerRace: 0, // Mặc định: Random mỗi luồng 8 - 16 xe độc bản, không trùng lặp
  ...
  const handleApplyConfig = (updates: Partial<SystemConfig>) => {
    setConfig(prev => {
      ...
      const carsLabel = updated.carsPerRace === 0 ? 'Random mỗi luồng (8 - 16 xe độc bản)' : `${updated.carsPerRace || 15} xe`;
