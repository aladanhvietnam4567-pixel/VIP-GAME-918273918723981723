...
import { Play, Square, Pause, HardDrive, Cpu, Film, Terminal, HelpCircle, Video, Loader2, Mic, MicOff, Globe, BookOpen } from 'lucide-react';
...
export const DashboardHeader: React.FC<DashboardHeaderProps> = ({
  ...
  const handleToggleCommentaryLanguage = () => {
    ...
    <header className="bg-slate-900 border-b border-slate-800 px-4 py-2.5 select-none">
      <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 w-full md:w-auto justify-center">
          ...
          {/* Nút Mở Kho 3,000 Kịch Bản Hài Hước */}
          {onOpenStorylines && (
            <button
              id="btn-open-storylines"
              onClick={onOpenStorylines}
              className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-gradient-to-r from-amber-950/70 to-slate-800 hover:from-amber-900/80 hover:to-slate-700 border border-amber-500/40 text-xs font-bold text-amber-300 transition cursor-pointer shadow-sm"
              title="Khám phá và xem thử kho 3,000 Cốt Truyện Kịch Bản Hài Hước cho các luồng đua"
            >
              <BookOpen className="w-4 h-4 text-amber-400" />
              <span>3,000 KỊCH BẢN</span>
            </button>
          )}

          {/* Bật/Tắt Bình Luận Viên & Chuyển đổi Ngôn ngữ Việt / Anh */}
          <div className="flex items-center rounded-lg border border-slate-700 bg-slate-800/90 overflow-hidden shadow-sm">
            <button
              id="btn-toggle-commentary"
              onClick={handleToggleCommentary}
              className={`flex items-center gap-1.5 px-3 py-2 text-xs font-bold transition active:scale-95 cursor-pointer ${
                isCommentaryOn
                  ? 'bg-amber-950/80 text-amber-300'
                  : 'text-slate-400 hover:text-slate-200'
              }`}
              title="Bật/Tắt Bình Luận Viên (Hỗ trợ Song Ngữ Tiếng Việt Hài Hước & English F1 Broadcast)"
            >
              {isCommentaryOn ? (
                <>
                  ...
                  <span>{commentaryLang === 'en' ? 'F1 BROADCAST' : 'BLV HÀI HƯỚC'}</span>
                </>
              ) : (
                <>
            ...
            <button
              id="btn-toggle-commentary-lang"
              onClick={handleToggleCommentaryLanguage}
              className="px-2 py-2 text-xs font-bold border-l border-slate-700/80 bg-slate-900/90 hover:bg-slate-700/80 text-cyan-300 transition cursor-pointer flex items-center gap-1"
              title="Chuyển đổi Ngôn Ngữ Bình Luận: Tiếng Việt 🇻🇳 vs English F1 🇬🇧"
            >
