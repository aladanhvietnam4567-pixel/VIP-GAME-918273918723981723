...
import { Settings, Folder, Video, Sliders, Camera, Sparkles, FolderPlus, Download, Smartphone, Film, Check, Users, Gauge, Mic, MicOff, Globe, Clapperboard } from 'lucide-react';
...
export const ConfigurationPanel: React.FC<ConfigurationPanelProps> = ({
  ...
  const [selectedCars, setSelectedCars] = useState<0 | 6 | 8 | 10 | 12 | 15>((config.carsPerRace as 0 | 6 | 8 | 10 | 12 | 15) || 15);
  ...
  const carScenarioOptions: { count: 0 | 6 | 8 | 10 | 12 | 15; label: string; desc: string; badge: string }[] = [
    { count: 0, label: '🎲 Random', desc: 'Mỗi luồng ngẫu nhiên 8 - 16 xe độc bản', badge: 'Khuyên Dùng' },
    { count: 6, label: '6 Xe', desc: 'Đua nhóm tinh gọn, bám đuổi kịch tính', badge: 'Gọn nhẹ' },
    { count: 10, label: '10 Xe', desc: 'Chuẩn Grand Prix, hỗn chiến cao độ', badge: 'Grand Prix' },
    { count: 15, label: '15 Xe', desc: 'Tối đa 15 xe, nghẹt thở nghẽn đường', badge: 'Tối đa' }
  ];
...
`;
  ...
  return (
    <aside className="w-full lg:w-80 bg-slate-900/95 border-b lg:border-b-0 lg:border-r border-slate-800 p-4 space-y-4 text-sm text-slate-300 shrink-0 overflow-y-auto">
      ...
      {/* 5. Bình Luận Viên Trực Tiếp & Ngôn Ngữ */}
      <div className="space-y-2.5 p-2.5 rounded-xl bg-slate-950/70 border border-slate-800">
        <div className="flex items-center justify-between">
          <label className="flex items-center gap-1.5 text-xs font-bold text-rose-300">
            <Mic className="w-3.5 h-3.5 text-rose-400" />
            Bình Luận Viên Đua Xe:
          </label>
          <button
        ...
        {/* Lựa chọn Ngôn ngữ Bình Luận: Tiếng Việt vs Tiếng Anh */}
        <div className="space-y-1">
          <div className="flex items-center justify-between text-[11px] text-slate-300 font-semibold">
            <span className="flex items-center gap-1">
              <Globe className="w-3 h-3 text-cyan-400" />
              Chế Độ Ngôn Ngữ Khán Giả:
            </span>
            <span className="font-mono text-[10px] text-cyan-400 font-bold uppercase">
              {commentaryLang === 'en' ? 'ENGLISH (F1 LEAD)' : 'TIẾNG VIỆT (VIRAL)'}
            </span>
          </div>

          <div className="grid grid-cols-2 gap-1.5">
            <button
              id="btn-commentary-lang-vi"
              onClick={() => {
              ...
              className={`py-1.5 px-2 rounded-lg text-xs font-bold transition cursor-pointer flex items-center justify-center gap-1.5 ${
                commentaryLang === 'vi'
                  ? 'bg-rose-600 text-white shadow-md shadow-rose-950 ring-1 ring-rose-400'
        ...
        <p className="text-[10.5px] text-slate-400">
          {commentaryLang === 'en'
            ? '🎙️ Chế độ Tiếng Anh F1 World Broadcast Feed: Bình luận viên chuyên nghiệp chuẩn quốc tế dành cho khán giả nước ngoài, phân tích góc quay và tình huống tốc độ cao!'
            : '🎙️ Chế độ Tiếng Việt Trẻ Trâu: Kho từ vựng hài hước, chế vui mỉa mai hóm hỉnh phù hợp người xem mạng xã hội TikTok / Reels / Shorts.'}
        </p>

        {/* Kịch bản Đạo diễn 10 Luồng Độc Bản */}
        <div className="pt-1.5 border-t border-slate-800/80">
          <div className="flex items-center gap-1 text-[11px] font-bold text-amber-300 mb-1">
            ...
            <span>Kịch Bản Đạo Diễn Riêng Biệt (10 Luồng):</span>
          </div>
          <p className="text-[10px] text-slate-400 leading-tight">
            Mỗi luồng game được điều khiển bởi 1 phong cách Đạo Diễn Hình Ảnh độc bản (F1 World Feed, Hollywood Action, Flycam Drone, Bumper Speed, Cyberpunk Drift...) tránh trùng lặp góc quay giữa các video.
          </p>
        </div>
      </div>
