          ...
          return (
            ...
            >
              ...
              >
                ...
                {/* Driver List Overlay - Professional Broadcast Starting Grid from Second 3 to Second 6 (Hiển thị chính xác & đầy đủ 6/15 siêu sao từ kho 60 cầu thủ) */}
                {instance.chunkTimeElapsed >= 3.0 && instance.chunkTimeElapsed <= 6.0 && (
                  <div className="absolute top-2 left-2 right-2 max-h-[88%] bg-slate-950/95 backdrop-blur-md z-40 p-2 flex flex-col rounded-lg border border-cyan-500/40 shadow-2xl animate-fade-in font-sans pointer-events-none overflow-hidden">
                    <div className="mb-1.5 border-b border-cyan-500/30 pb-1 shrink-0 flex items-center justify-between">
                      <div>
                        <div className="text-[7.5px] font-mono tracking-[0.1em] text-cyan-400 font-bold uppercase flex items-center gap-1">
                          ...
                          STARTING GRID • XUẤT PHÁT
                        </div>
                        <h3 className="text-[9.5px] font-black text-white uppercase tracking-wide">
                          {instance.cars.length} Siêu Sao Sân Cỏ Đua Tốc Độ
                        </h3>
                        <p className="text-[6.5px] text-slate-400 font-mono">
                          LUỒNG #{instance.id.toString().padStart(2, '0')} • POOL 60 CẦU THỦ THẾ GIỚI
                        </p>
                      </div>
                      <div className="bg-cyan-950/80 border border-cyan-500/30 px-1.5 py-0.5 rounded text-[7px] font-mono font-bold text-cyan-300">
                        {instance.cars.length}/15 XE
                      </div>
                    </div>

                    {/* Danh sách chính xác và đầy đủ các tay đua trong cuộc đua */}
                    <div className={`grid ${instance.cars.length > 8 ? 'grid-cols-2 gap-x-1.5 gap-y-0.5' : 'flex flex-col gap-1'} overflow-hidden`}>
                      {[...instance.cars]
                        ...
                        .map((car, idx) => {
                          ...
                          return (
                            <div
                              key={car.id || idx}
                              className="flex items-center justify-between bg-slate-900/90 border border-slate-800/70 rounded px-1 py-0.5 shrink-0"
                            >
                              <div className="flex items-center gap-1 min-w-0 truncate">
                                {/* Grid Position */}
                                <div className="w-3.5 h-3.5 rounded bg-cyan-950 border border-cyan-500/30 flex items-center justify-center font-mono font-black text-cyan-400 text-[7px] shrink-0">
                                  {idx + 1}
                                </div>
                                ...
                                {/* Driver and Car Name */}
                                <div className="flex flex-col min-w-0 truncate leading-none">
                                  <span className="font-bold text-white uppercase text-[7.5px] tracking-tight truncate">
                                    {car.driverName || `Tay Đua #${idx + 1}`}
                                  </span>
                                  <span className="text-[6px] text-slate-400 font-mono truncate">
                                    {car.name || `GT Supercar #${idx + 1}`}
                                  </span>
                                </div>
                              </div>

                              {/* Category Badge */}
                              <div className="text-[5.5px] font-mono font-bold text-cyan-400/90 bg-cyan-950/40 px-1 rounded border border-cyan-500/10 shrink-0 ml-1">
                                {car.type === 'formula' ? 'F1' : 'HYPER'}
                              </div>
                            </div>
                          );
                        })}
