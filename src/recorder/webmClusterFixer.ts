...
function concatBuffers(buffers: (Uint8Array | Uint8Array<ArrayBufferLike>)[]): Uint8Array {
  let totalLen = 0;
...
export async function expandWebmClustersAndDuration(
  ...
  try {
    ...
    interface ClusterEntry {
      ...
      payloadRest: Uint8Array | Uint8Array<ArrayBufferLike>;
    }
    ...
    while (segPos < rawBuf.length) {
      ...
      if (idInfo.id === 0x1654AE6B) { // Tracks
        ...
        let tcVal = 0;
        let payloadRest: Uint8Array | Uint8Array<ArrayBufferLike> = clusterPayload;
    ...
    return new Blob([finalBuffer.buffer as ArrayBuffer], { type: blob.type || 'video/webm' });
  } catch (err) {
