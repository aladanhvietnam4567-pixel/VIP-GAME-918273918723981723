...
export class VideoRecorderService {
  ...
  public drawBroadcastHUD(
    ...
    // Thiết kế chuẩn truyền hình F1/Esports đỉnh cao: Hiển thị chính xác & đầy đủ 6/15 siêu sao ngẫu nhiên từ kho 60 cầu thủ
    if (elapsedSecs >= 3.0 && elapsedSecs <= 6.0) {
      ctx.save();
      const participatingCars = [...instance.cars].sort((a, b) => (a.state.rank || 0) - (b.state.rank || 0));
      const totalDrivers = participatingCars.length;
      const isTwoColumn = totalDrivers > 8;

      const cardW = isTwoColumn ? 980 : 640;
      const cardX = Math.round((1080 - cardW) / 2);
      const cardY = 200;
      const rowCount = isTwoColumn ? Math.ceil(totalDrivers / 2) : totalDrivers;
      const rowH = isTwoColumn ? 60 : 68;
      const cardH = 130 + rowCount * rowH;

      // Nền kính tối mờ cao cấp
      ctx.fillStyle = 'rgba(7, 11, 22, 0.92)';
      ctx.beginPath();
      if ((ctx as any).roundRect) {
        (ctx as any).roundRect(cardX, cardY, cardW, cardH, 18);
      } else {
      ...
      ctx.textAlign = 'left';
      ctx.fillText(`● STARTING GRID • DANH SÁCH XUẤT PHÁT (${totalDrivers} TAY ĐUA)`, cardX + 24, cardY + 36);

      ctx.fillStyle = '#ffffff';
      ctx.font = 'bold 24px sans-serif';
      ctx.fillText(`${totalDrivers} SIÊU SAO SÂN CỎ BỨT TỐC ĐỈNH CAO`, cardX + 24, cardY + 70);

      ctx.fillStyle = '#94a3b8';
      ctx.font = '13.5px monospace';
      ctx.fillText(`LUỒNG ĐUA #${instance.id.toString().padStart(2, '0')} • POOL 60 CẦU THỦ THẾ GIỚI • SEED #${seed}`, cardX + 24, cardY + 95);
      ...
      const colW = isTwoColumn ? 465 : cardW - 32;

      participatingCars.forEach((car, idx) => {
        const colIdx = isTwoColumn ? Math.floor(idx / rowCount) : 0;
        const rowIdx = isTwoColumn ? (idx % rowCount) : idx;
        const x = cardX + 16 + colIdx * (colW + 18);
        const y = startY + rowIdx * rowH;
        const h = rowH - 10;

        const driverName = car.state.driverName || `Tay Đua #${idx + 1}`;
        const carName = car.state.name || `GT Supercar #${idx + 1}`;
        const colorHex = car.state.hexColor !== undefined
          ? `#${car.state.hexColor.toString(16).padStart(6, '0')}`
          : (idx % 5 === 0 ? '#38bdf8' : idx % 5 === 1 ? '#dc2626' : idx % 5 === 2 ? '#eab308' : idx % 5 === 3 ? '#2563eb' : '#16a34a');

        // Row background
        ctx.fillStyle = 'rgba(15, 23, 42, 0.85)';
        ctx.beginPath();
        if ((ctx as any).roundRect) {
          (ctx as any).roundRect(x, y, colW, h, 8);
        } else {
          ctx.rect(x, y, colW, h);
        }
        ctx.fill();
        ...
        ctx.fillRect(x + 6, y + 6, 36, h - 12);
        ctx.strokeStyle = 'rgba(6, 182, 212, 0.4)';
        ctx.strokeRect(x + 6, y + 6, 36, h - 12);

        ctx.fillStyle = '#06b6d4';
        ctx.font = 'bold 16px monospace';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(`P${idx + 1}`, x + 24, y + h / 2);

        // Color Pill
        ctx.fillStyle = colorHex;
        ctx.fillRect(x + 48, y + 6, 5, h - 12);

        // Driver Name
        ctx.fillStyle = '#ffffff';
        ctx.font = isTwoColumn ? 'bold 15px sans-serif' : 'bold 17px sans-serif';
        ctx.textAlign = 'left';
        ctx.textBaseline = 'top';
        ctx.fillText(driverName.toUpperCase(), x + 60, y + 7);

        // Car Subtitle
        ctx.fillStyle = '#94a3b8';
        ctx.font = isTwoColumn ? '11px monospace' : '12.5px monospace';
        ctx.fillText(carName.toUpperCase(), x + 60, y + 27);

        // Category Badge
        ctx.fillStyle = 'rgba(6, 182, 212, 0.2)';
        ctx.fillRect(x + colW - 56, y + (h - 24) / 2, 48, 24);
        ctx.strokeStyle = 'rgba(6, 182, 212, 0.4)';
        ctx.strokeRect(x + colW - 56, y + (h - 24) / 2, 48, 24);

        ctx.fillStyle = '#06b6d4';
        ctx.font = 'bold 11px monospace';
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText(car.state.type === 'formula' ? 'F1' : 'HYPER', x + colW - 32, y + h / 2);
      });
    ...
    // 🎙️ Phụ đề bình luận viên truyền hình (Song ngữ Tiếng Việt & English F1)
    try {
      const commentaryLang = commentaryEngine.getLanguage();
      const participatingNames = instance.cars
        .map(c => c.state.driverName)
        .filter(Boolean) as string[];
      const commentaryTimeline = commentarySoundManager.getTimelineForInstance(
        instance.id,
        seed,
        totalSecs,
        commentaryLang,
      ...
      if (activeCommentary) {
        ...
        ctx.strokeStyle = commentaryLang === 'en' ? '#0284c7' : '#ef4444';
        ctx.lineWidth = 2.5;
        ctx.strokeRect(40, subY, 1000, 120);

        // Badge BLV TRẺ TRÂU / F1 LEAD COMMENTARY
        const isEn = commentaryLang === 'en';
        ctx.fillStyle = isEn ? '#0284c7' : '#ef4444';
        ctx.fillRect(52, subY + 12, isEn ? 200 : 195, 26);
        ctx.fillStyle = '#ffffff';
        ctx.font = 'bold 13px sans-serif';
        ctx.fillText(isEn ? '🎙️ F1 BROADCAST' : '🎙️ BLV TRẺ TRÂU', 58, subY + 30);
